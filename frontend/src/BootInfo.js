...
function BootInfo() {
    ...
    useEffect(() => {
        const fetchMemInfo = () => {
            axios
                .get('https://dfs-backend.onrender.com//boot-info')
                .then((response) => {
                    // console.log('res boot info =  ', response.data.bootInfo);
                    setBootInfo(response.data.bootInfo);
                })
                .catch((error) => {
    ...
    return (
        <div class="max-h-[50vh] overflow-auto m-auto mt-2 max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <a href="#">
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-blue-500">Boot-Info</h5>
            </a>
            <button
            ...
            <div className="text-gray-800 dark:text-white text-left">
                {Object.entries(bootInfo).map(([sectionName, sectionData], index) => (
                    <div key={index}>
